...
def Inscription(root):
	...
	#e1 = Entry(fenetre1)
	e1=ttk.Combobox(frame1,values=ListeGenre)  #genre
	e2 = Entry(frame1) # Nom
	e3 = Entry(frame1) # Prenom
	e4 = Entry(frame1) # Date de naissance
	e44 = Entry(frame1) # NNI
	e444 = Entry(frame1) # Frais d'inscription
	e4444=ttk.Combobox(frame1,values=ListeAnneeScolaire) # Annee Scoalire
	# frame2
	e5 = ttk.Combobox(frame2,values=ListeEtablissement)    # Etablissement
	e6 = ttk.Combobox(frame2,values=ListeClasse)	  # Classe
	e66 = Entry(frame2)  # N° eleve
	e7 = Entry(frame2)  # Adresse
	
	# frame3

	e8 = Entry(frame3) # Login
	e9 = Entry(frame3) # MP
	e10 = Entry(frame3) # Confirme MP
	e11= Entry(frame3) # E-Mail
	e12= Entry(frame3) # N° Tel

	# frame4

	e13= Entry(frame4)  # Tuteur 1
	e14= Entry(frame4)  # Tel Tuteur 1
	e15= Entry(frame4)  # Adresse Tuteur 1
	
	#frame 5
	
	e16= Entry(frame5) # Tuteur 2
	e17= Entry(frame5)  # Tel Tuteur 2
	e18= Entry(frame5) # Adresse Tuteur 2
	...
	def ecriture():
		...
		try :
			...
			psw=hasher.encode(password=e9.get(), salt='salt',iterations=50000)

			#print(psw)
			e=datetime.now()

			conn =sqlite3.connect('C:/Users/Demba/Desktop/serveur/E_SCHOOL/db.sqlite3')
			...
			VALUES(?,?,?,?,?,?,?,?,?,?)
			''',(psw, 'NULL', 0, e8.get(), e2.get(), e11.get(), 0, 1, e, e3.get()))
